...
def _numeric_stats(series: pd.Series) -> dict:
    stats: dict = {}
    stats["mean"] = _round_stat(series.mean())
    stats["std"] = _round_stat(series.std())
    stats["min"] = _safe_json_value(series.min())
    stats["max"] = _safe_json_value(series.max())
...
def _boolean_stats(series: pd.Series) -> dict:
    ...
    return {
        ...
        "true_pct": _round_stat(100.0 * true_count / len(series)),
    }
...
def _object_stats(series: pd.Series) -> dict:
    ...
    if len(vc) > 0:
        stats["top"] = _safe_json_value(vc.index[0])
...
def _categorical_stats(series: pd.Series) -> dict:
    ...
    if len(cats) <= 20:
        stats["categories"] = [_safe_json_value(c) for c in cats]
    return stats
...
def _sample_rows(df: pd.DataFrame, n: int) -> list[dict]:
    ...
    for row_idx in range(len(sample)):
        ...
        for col_idx in range(sample.shape[1]):
            ...
            key = str(col_name)
            row[key] = _safe_json_value(sample.iloc[row_idx, col_idx])
        rows.append(row)
    return rows
...
def _round_stat(val) -> float | None:
